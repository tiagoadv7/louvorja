...
// (AboutDialog.vue). Sem cor de marca fixa por ícone (facebook azul,
// instagram roxo, etc.) — o ícone usa a cor "primary" do tema ATUAL (ver
// AboutDialog.vue), pra ficar consistente com qualquer tema escolhido em
// vez de sempre mostrar as mesmas cores de marca.
export function contributorLinks(c) {
  ...
  if (c.linkedin) links.push({ icon: 'mdi-linkedin', url: `https://linkedin.com/in/${c.linkedin}`, label: `linkedin.com/in/${c.linkedin}` });
  if (c.facebook) {
    const url = c.facebook.startsWith('http') ? c.facebook : `https://facebook.com/${c.facebook}`;
    links.push({ icon: 'mdi-facebook', url, label: url.replace(/^https?:\/\//, '') });
  }
  if (c.instagram) links.push({ icon: 'mdi-instagram', url: `https://instagram.com/${c.instagram}`, label: `instagram.com/${c.instagram}` });
  ...
  if (c.whatsapp) links.push({ icon: 'mdi-whatsapp', url: `https://wa.me/${c.whatsapp}`, label: 'WhatsApp' });
